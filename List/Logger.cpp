...
bool Logger::Log(std::string line)
{
    ...
    memory[currentLocMemory % 100] = memLine;
    currentLocMemory++;
...
void Logger::PurgeMemoryLog()
{
    ...
    {
        //Print everything that was logged first
        for (int i = ((currentLocMemory % 100) + 1); i < 100; i++)
        {
            std::cout << memory[i] << std::endl;
            memory[i] = "";
        }
        //Print everything that was logged last
        for (int i = 0; i <= ((currentLocMemory % 100) - 1); i++)
        {
            std::cout << memory[i] << std::endl;
            memory[i] = "";
        }
